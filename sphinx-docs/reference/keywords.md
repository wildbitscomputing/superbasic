...
Binary modulus operator. Returns the non-negative remainder. The second value
must be non-zero.
...
division, forward slash returns a floating point value.
...
be used to compare two numbers or two strings.
...
Returns the address of an l-expr, normally this is a variable of some sort, but
it can be an array element or even an indirection.
...
Binary AND operator. This is a binary operator not a logical one, so it can
return values other than true and false.
...
Binary OR operator. This is a binary operator not a logical one, so it can
return values other than true and false.
...
Sends a three-parameter command directly to the graphics subsystem. Often the
last two parameters are coordinates (not always). It is not advised to use this
for general use as programs would be somewhat unreadable.
...
Inputs numbers or strings from the keyboard. This version uses the same syntax
as `print`, except that where there is a variable a value is entered into that
variable.
...
Assignment statement. The `LET` is optional. You can also use `@a` where `a` is
a reference.
...
Draws a line, using the standard syntax which is explained in the graphics
section.
...
Lists the program. It is possible to list any part of the program using the line
numbers, or list a procedure by name.
...
Returns a subsegment of a string, given the start position (first character is
1\) and the length, so `mid$("abcdef",3,2)` returns `"cd"`.
...
Gets the current status of the PS/2 mouse. 6 reference parameters (normally
integer variables) are provided: current mouse position in the x, y, z axes, and
the status of the left, middle and right buttons.
...
Unary operator returning the logical NOT of its parameter, i.e. 0 if non-zero,
−1 otherwise.
...
Sets the graphics palette. The parameters are the colour id and the red, green
and blue components. On start up, the palette is `RRRGGGBB`.
...
The `poke`, `pokew`, `pokel` and `poked` functions write one to four bytes to
the 6502 memory space.
...
Returns the character at the given screen position (row, column) as a
...
Sets the RTC to the given date; the parameters are the day, month and year
(00–99).
...
and `sprite off` which turns a sprite off. For `sprite .. to` the sprite is
...
data in memory (default `$30000`). When turned on, all the sprites are erased
and their control values set to zero.
...
Manipulates the tile map. This allows you to set the scroll offset (with
`TO xscroll,yscroll`) and draw on the tile map using `AT x,y` to set the
position and `DRAW` followed by a list of tiles, with a repeat option using
`LINE`.
...
This command is a defensive measure against potential bugs in either the kernel,
the kernel drivers, or BASIC itself.
